...
async def lifespan(app: FastAPI):
    ...
    print(f"🔍 STRAVA_CLIENT_SECRET: {'***' if os.getenv('STRAVA_CLIENT_SECRET') else 'None'}")
    print(f"🔍 STRAVA_REDIRECT_URI: {os.getenv('STRAVA_REDIRECT_URI')}")
    print(f"🎵 SPOTIFY_CLIENT_ID: {os.getenv('SPOTIFY_CLIENT_ID')}")
    print(f"🎵 SPOTIFY_CLIENT_SECRET: {'***' if os.getenv('SPOTIFY_CLIENT_SECRET') else 'None'}")
...
def run_migrations():
    ...
    try:
        ...
        if 'profile_image' not in columns:
            print("🔄 Adding profile_image column to spotify_profiles table...")
            cursor.execute("ALTER TABLE spotify_profiles ADD COLUMN profile_image TEXT")
        
        if 'followers_count' not in columns:
            print("🔄 Adding followers_count column to spotify_profiles table...")
        ...
        print(f"⚠️ Warning: Some migrations failed: {e}")
        conn.rollback()
    finally:
        conn.close()
...
async def strava_auth():
    ...
    try:
        print(f"🔍 Generating auth URL with:")
...
async def strava_callback(code: str, state: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            print(f"❌ Token save error: {token_error}")
            raise token_error
        
        # Get athlete info
        print("��‍♀️ Getting athlete info...")
        try:
            athlete = strava_api.get_athlete(tokens['access_token'])
            print(f"✅ Athlete info received: {athlete.get('firstname', 'Unknown')} {athlete.get('lastname', 'Unknown')}")
        except Exception as athlete_error:
            print(f"❌ Athlete fetch error: {athlete_error}")
            raise athlete_error
        
        # Save athlete info to database
        print("💾 Saving athlete info to database...")
        try:
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("""
            ...
            conn.commit()
            conn.close()
            print("✅ Athlete info saved successfully")
...
async def get_strava_athlete():
    ...
    try:
        ...
        print(f"🔍 Checking tokens for user {user['id']}")
        tokens = strava_sync.get_valid_tokens(user['id'])
        if not tokens:
            print("❌ No valid tokens found")
            raise HTTPException(status_code=401, detail="Strava not connected")
        
        print(f"✅ Tokens found, fetching athlete info...")
        # Get athlete info
        athlete = strava_api.get_athlete(tokens['access_token'])
        print(f"✅ Athlete info retrieved: {athlete.get('firstname', 'Unknown')}")
        
        return athlete
    except Exception as e:
        print(f"❌ Error in get_strava_athlete: {e}")
        import traceback
        print(f"❌ Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
...
async def get_strava_activities(
    ...
    try:
        ...
        print(f"🔍 Checking tokens for user {user['id']} to get activities")
        tokens = strava_sync.get_valid_tokens(user['id'])
        if not tokens:
            print("❌ No valid tokens found for activities")
            raise HTTPException(status_code=401, detail="Strava not connected")
        
        print(f"✅ Tokens found, fetching activities (page {page}, per_page {per_page})...")
...
async def clear_strava_tokens():
    ...
    try:
        ...
        if user:
            ...
            cursor.execute("DELETE FROM strava_activities WHERE user_id = ?", (user['id'],))
            conn.commit()
            print("🧹 Cleared all Strava data for troubleshooting")
        
        return {"message": "Strava tokens and data cleared successfully"}
    except Exception as e:
        print(f"❌ Error clearing Strava data: {e}")
        return {"error": str(e)}
    finally:
        if conn:
            conn.close()
...
async def get_strava_status():
    ...
    try:
        ...
        print(f"❌ Error in get_strava_status: {e}")
...
async def spotify_callback(code: str):
    ...
    try:
        print(f"🎵 Spotify OAuth callback received with code: {code[:10]}...")
        ...
        # Save tokens to database
        print("💾 Saving Spotify tokens to database...")
        try:
            spotify_sync.save_tokens(user['id'], tokens)
            print("✅ Spotify tokens saved successfully")
        except Exception as token_error:
            print(f"❌ Token save error: {token_error}")
            raise token_error
        
        # Get user profile
        print("🎵 Getting Spotify user profile...")
        try:
            profile = spotify_api.get_user_profile(tokens['access_token'])
            print(f"✅ Profile received: {profile.get('display_name', 'Unknown')}")
        except Exception as profile_error:
            print(f"❌ Profile fetch error: {profile_error}")
            raise profile_error
        
        # Save profile to database
        print("💾 Saving Spotify profile to database...")
        try:
            spotify_sync.save_user_profile(user['id'], profile)
            print("✅ Spotify profile saved successfully")
        except Exception as db_error:
            print(f"❌ Database save error: {db_error}")
            raise db_error
        
        print("🎉 Spotify OAuth flow completed successfully!")
        return {
...
async def spotify_frontend_callback(code: str):
    ...
    try:
        print(f"🎵 Spotify frontend callback received with code: {code[:10]}...")
        ...
        print(f"❌ Error in Spotify frontend callback: {e}")
        print(f"❌ Error type: {type(e)}")
        import traceback
        print(f"❌ Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_spotify_profile():
    ...
    try:
        ...
        print(f"🔍 Checking Spotify tokens for user {user['id']}")
        tokens = spotify_sync.get_valid_tokens(user['id'])
        if not tokens:
            print("❌ No valid Spotify tokens found")
            raise HTTPException(status_code=401, detail="Spotify not connected")
        
        print(f"✅ Spotify tokens found, fetching profile...")
        # Get profile from Spotify
        profile = spotify_api.get_user_profile(tokens['access_token'])
        print(f"✅ Spotify profile retrieved: {profile.get('display_name', 'Unknown')}")
        
        return profile
    except Exception as e:
        print(f"❌ Error in get_spotify_profile: {e}")
        import traceback
        print(f"❌ Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_spotify_top_tracks(
    ...
    try:
        ...
        print(f"🔍 Checking Spotify tokens for user {user['id']} to get top tracks")
        tokens = spotify_sync.get_valid_tokens(user['id'])
        if not tokens:
            print("❌ No valid Spotify tokens found for top tracks")
            raise HTTPException(status_code=401, detail="Spotify not connected")
        
        print(f"✅ Spotify tokens found, fetching top tracks (time_range: {time_range}, limit: {limit})...")
        ...
        try:
            ...
            print("✅ Top tracks saved to database")
        except Exception as save_error:
            print(f"⚠️ Warning: Could not save tracks to database: {save_error}")
        
        return tracks
    except Exception as e:
        print(f"❌ Error in get_spotify_top_tracks: {e}")
        import traceback
        print(f"❌ Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_spotify_status():
    ...
    try:
        ...
        print(f"❌ Error in get_spotify_status: {e}")
        return {"connected": False, "error": str(e)}
...
async def disconnect_spotify():
    ...
    try:
        ...
        if user:
            ...
            conn.commit()
            print("🧹 Cleared all Spotify data")
...
async def clear_spotify_data():
    ...
    try:
        ...
        if user:
            ...
            cursor.execute("DELETE FROM spotify_tracks WHERE user_id = ?", (user['id'],))
            conn.commit()
            print("🧹 Cleared all Spotify data for troubleshooting")
